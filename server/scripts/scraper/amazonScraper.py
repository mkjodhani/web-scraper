...
class Scraper:
    ...
    def savetoSQL(self):
        ...
        text = "\n\n".join(statements)
        file = open(self.location.replace(self.location.split("/")[-1],"")+self.filename+".sql","w")
        file.write(text)
        file.close()
...
<script>
  ...
  for(var i in rows){
    ...
    rows[i].getElementsByTagName('td')[2].appendChild(img);
        ...
        file = open(self.location.replace(self.location.split("/")[-1],"")+self.filename+".html","w")
        file.write(html)
        file.close()

    def saveToFile(self,ext):
        # if ext == "JSON":
            self.dataFrame.to_json(self.location.replace(self.location.split("/")[-1],"")+self.filename+".json")
        # elif ext == "CSV":
            self.dataFrame.to_csv(self.location.replace(self.location.split("/")[-1],"")+self.filename+".csv")
        # elif ext == "EXCEL":
            self.dataFrame.to_excel( self.location.replace(self.location.split("/")[-1],"")+self.filename+".xlsx", sheet_name=self.filename)
        # elif ext == "SQL":
            self.savetoSQL()
        # elif ext == "HTML":
            self.makeHTML()
            ...
            for p_name in product_name:
                ...
                if(len(images) != 0 and len(price) != 0):
                    ...
                    if(product_name_from_web not in product_names and download_path not in productImages):
                        ...
                        row = pd.Series(data=[product_ID,product_name_from_web,price_value,f'{ self.location.split("/")[-1]+"/"+imagePath}',download_path],index=["ID","Name","Price","Image","Image URL"],name=product_ID)
